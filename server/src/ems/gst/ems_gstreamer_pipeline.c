...
benchmark_down_msg_loss(struct ems_gstreamer_pipeline *self, GstMapInfo* map_info) {
	...
	if (!result) {
		U_LOG_E("Decoding protobuf failed: %s downMsg_bytes size: %ld", PB_GET_ERROR(&our_istream), map_info->size);
	} else {
		...
		self->sent_down_msg_list = g_slist_prepend(self->sent_down_msg_list, GINT_TO_POINTER((gint)msg.frame_data.frame_sequence_id));
		...
		if (duration_since_last_print_secs >= 5.0) {
			...
			while (l != NULL) {
				...
				if (last != -1) {
					...
					if (num_skipped > 0) {
						// U_LOG_D("Skipped payloading %d DownMsgs (%d - %d)", num_skipped, last + 1, current - 1);
